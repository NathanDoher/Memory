...
if p==1                  %Player 1 options and player 2 options
    y=1;                 %Sets the ratio of the buttons to full screen
...
elseif p==2
    ...
    S.pt_n1 = uicontrol('style','text','unit','normalized','fontsize',15,...
        ...
        'HorizontalAlignment','left','ForegroundColor',name{3});
    ...
    S.pt_n2 = uicontrol('style','text','unit','normalized','fontsize',15,...
        ...
        'HorizontalAlignment','left','ForegroundColor',name{4});
...
uimenu(memMenu,'Label','1-Player','Callback',@playOne);
...
%%%%Push Button function was adapted and heavily changed from GUI_9%%%%
...
%%%%Adapted the Enable option from the memory game cited, in order to pass whos turn it is.%%%%
...
elseif p==2
    aCell={['Player ' get(S.pt_n1,'string') ' Scored:'],get(S.pt_s1,'string'),...
        ...
        'Total time played', toc, 'seconds'};
end

if p==1 && matSize==3
                NAME = input('Please Enter Your Name: ', 's');
                Leaderboard = fopen('EasyLeaderboard.txt', 'w');
                fprintf(Leaderboard, 'Name:            Attempts: \n');
                fprintf(Leaderboard,'%s          %5.0f \n', NAME, counter);
end

if p==1 && matSize==4 
                NAME = input('Please Enter Your Name: ', 's');
                Leaderboard = fopen('MediumLeaderboard.txt', 'w');
                fprintf(Leaderboard, 'Name:            Attempts: \n');
                fprintf(Leaderboard,'%s          %5.0f \n', NAME, counter);
end
...
if p==1 && matSize==6 
                NAME = input('Please Enter Your Name: ', 's');
                Leaderboard = fopen('HardLeaderboard.txt', 'w');
                fprintf(Leaderboard, 'Name:            Attempts: \n');
                fprintf(Leaderboard,'%s          %5.0f \n', NAME, counter);
end

S.fhe = figure('units','normalized','position',[.3 .3 .3 .4],'menubar','none','name','Game Over','numbertitle','off','resize','off');
...
end %function [] = playOne(~,~)


%-----------------Two Player Options-----------------
function [] = playTwo(~,~)

S.fh = figure('units','normalized','position',[.3 .3 .25 .20],'menubar','none','name','Custom Options','numbertitle','off','resize','off');
S.pt1 = uicontrol('style','text','unit','normalized','fontsize',14,'position',[0 .8 1 .2],'string','Please enter names of players.','backgroundc',[0.94 .94 .94]);

S.pt1 = uicontrol('style','text','unit','normalized','fontsize',14,'position',[0 .6 .3 .2],'string','Player 1','backgroundc',[0.94 .94 .94]);
S.pt2 = uicontrol('style','text','unit','normalized','fontsize',14,'position',[0 .4 .3 .2],'string','Player 2','backgroundc',[0.94 .94 .94]);

S.pe1 = uicontrol('style','edit','unit','normalized','fontsize',14,'position',[.3 .6 .4 .2],'string','','backgroundc',[0.94 .94 .94]);
S.pe2 = uicontrol('style','edit','unit','normalized','fontsize',14,'position',[.3 .4 .4 .2],'string','','backgroundc',[0.94 .94 .94]);

S.pp1 = uicontrol('style','popupmenu','unit','normalized','fontsize',14,'position',[.7 .6 .3 .2],'string',{'Red' 'Blue' 'Yellow' 'White' 'Green' 'Orange'},'backgroundc',[0.94 .94 .94]);
S.pp2 = uicontrol('style','popupmenu','unit','normalized','fontsize',14,'position',[.7 .4 .3 .2],'string',{'Red' 'Blue' 'Yellow' 'White' 'Green' 'Orange'},'backgroundc',[0.94 .94 .94]);

S.pb = uicontrol('style','push','unit','normalized','fontsize',14,'position',[.3 0 .4 .25],'string','Okay','backgroundc',[0.94 .94 .94],'callback',{@twoProcess});

end %function [] = playTwo(~,~)

function [] = twoProcess(~,~)

name1=get(S.pe1,'string');
name2=get(S.pe2,'string');

color1=get(S.pp1,'value');
if color1 == 1
   color1=[1 0 0];%Red
elseif color1 == 2
   color1=[0 0 1];%Blue
...
playerCount=2;
n={name1 name2 color1 color2};

close all

Memory(matSize,playerCount,n)

end %function [] = twoProcess(~,~)
...
itext1= 'Please Enter a Number for a Custom Game, 1 is easiest.  The number entered is the number of rows and columns created, to create square playing field.';
itext2= '*Note that some sizes may require the window to be resized, or may not work for your computer.';
S.fh = figure('units','normalized','position',[.3 .3 .3 .3],'menubar','none','name','Custom Options','numbertitle','off','resize','on');
...
function [] = help_play(~,~)
itext1= '1 Player games, simply try to match the left side of the board with the right side board.  2 Player game, match left side with right side.  Correct attempts allow the user to contiue to play.  Incorrect attempts, current player loses the turn and switches to the other player';
S.fhelp = figure('units','normalized','position',[.3 .3 .25 .25],'menubar','none','name','How To Play','numbertitle','off','resize','on');
S.pt1 = uicontrol('style','text','unit','normalized','fontsize',15,'position',[0 0 1 1],'string',itext1,'backgroundc',[0.94 .94 .94]);
end %function [] = help_play(~,~)

function [] = about(~,~)
itext1= 'This game was created by: Kenny Ben Isaac and Nate on 2016';
S.fhelp = figure('units','normalized','position',[.3 .3 .25 .25],'menubar','none','name','How To Play','numbertitle','off','resize','on');
S.pt1 = uicontrol('style','text','unit','normalized','fontsize',15,'position',[0 0 1 1],'string',itext1,'backgroundc',[0.94 .94 .94]); 
end %function [] = about(~,~)
